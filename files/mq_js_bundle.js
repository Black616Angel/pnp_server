...
function audio_init () {
  if (audio_context == null) {
    ...
    ;(a.buffer = audio_context.createBuffer(1, 1, 22050)),
      a.connect(audio_context.destination),
      a.start(0)
  }
}
...
function audio_play_buffer (f, g, h, j, i) {
  ...
  let b = null,
    ...
    e = null
  if (g != 1 || h != 1) {
    ;(b = audio_context.createGain()),
      a.connect(b),
      (c = audio_context.createGain()),
      a.connect(c)
    let e = audio_context.createChannelMerger(2)
    b.connect(e, 0, 0),
      c.connect(e, 0, 1),
      e.connect(audio_context.destination),
      (b.gain.value = g),
      (c.gain.value = h),
      (d.merger = e)
...
function audio_source_set_volume (a, d, e) {
  ...
  b[0].gain.linearRampToValueAtTime(d, c),
    b[1].gain.linearRampToValueAtTime(e, c)
}
